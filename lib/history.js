...
History.prototype.getTicketNumber = function()
{
    var ticketNumbers = _( this._items )
        .sortByOrder( 'height', false )
        .filter(function (item)
        {
            return item.block.ticketNumber;
        })
        .slice(0, MAX_BINS)
        .reverse()
        .map(function (item)
...
History.prototype.getmyTicketNumber = function()
{
    var ticketNumbers = _( this._items )
        .sortByOrder( 'height', false )
        .filter(function (item)
        {
            return item.block.myTicketNumber;
        })
        .slice(0, MAX_BINS)
        .reverse()
        .map(function (item)
...
History.prototype.getCharts = function()
{
	if(this._callback !== null)
	{
		var chartHistory = _( this._items )
			...
			.map(function (item)
			{
                return {
					...
					ticketNumber: item.block.ticketNumber,
		...
		this._callback(null, {
            ...
            myTicketNumber : this.getmyTicketNumber()
		});
	}
}
